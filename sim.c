...
void sim_init( sim_t* sim ) {
    ...
    sim->lenX = 256;
    sim->lenY = 256;
    sim->channelR = allocateArray1DInt(sim->nSpecies + 1);
    sim->channelG = allocateArray1DInt(sim->nSpecies + 1);
    sim->channelB = allocateArray1DInt(sim->nSpecies + 1);
    memcpy(sim->channelR, (int []){0, 0, 200, 0}, (sim->nSpecies + 1)*sizeof(int));
    memcpy(sim->channelG, (int []){0, 200, 0, 0}, (sim->nSpecies + 1)*sizeof(int));
    memcpy(sim->channelB, (int []){0, 0, 0, 200}, (sim->nSpecies + 1)*sizeof(int));
